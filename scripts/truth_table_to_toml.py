...
def parse_pla(path: Path) -> tuple[list[str], list[str], list[dict[str, str]]]:
    """
    Parse an Espresso PLA file with .ilb/.ob headers (e.g. generated by yosys write_pla).
    ...
    Data lines have the form "<input_bits> <output_bits>" where:
      - input_bits: binary string, no don't-cares (full truth table via abc collapse)
      - output_bits: binary string, '-' treated as don't-care ('X')
    """
    text = path.read_text(encoding="utf-8")
    lines = text.splitlines()
