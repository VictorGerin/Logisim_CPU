...
def _expand_input_dontcares(
    in_str: str,
    out_str: str,
    input_signals: list[str],
    output_signals: list[str],
) -> list[dict[str, str]]:
    """Expand a PLA row with input don't-cares into one row per combination.

    E.g. '0-1 10' expands to two rows: '001 10' and '011 10'.
    Output don't-cares ('-') are kept as 'X' (chip tester: don't check).
    """
    dc_positions = [i for i, c in enumerate(in_str) if c not in ("0", "1")]
    n_dc = len(dc_positions)
    result: list[dict[str, str]] = []
    for combo in range(1 << n_dc):
        chars = list(in_str)
        for bit_idx, pos in enumerate(dc_positions):
            chars[pos] = "1" if (combo >> (n_dc - 1 - bit_idx)) & 1 else "0"
        row: dict[str, str] = {}
        for i, sig in enumerate(input_signals):
            row[sig] = chars[i]
        for i, sig in enumerate(output_signals):
            c = out_str[i]
            row[sig] = "X" if c in ("-", "x", "X") else (c if c in ("0", "1") else "X")
        result.append(row)
    return result


def parse_pla(path: Path) -> tuple[list[str], list[str], list[dict[str, str]]]:
    """
    Parse an Espresso PLA file with .ilb/.ob headers.
    Returns (input_signals, output_signals, list of row dicts) in the same format as
    parse_truth_table(), so downstream code (build_vectors, write_toml) works unchanged.

    Input don't-cares ('-') are expanded into all combinations so that every
    test vector has a specific driven value on every input pin.
    Output don't-cares ('-') become 'X' (chip tester: don't check).
    """
    text = path.read_text(encoding="utf-8")
    lines = text.splitlines()
    input_signals: list[str] = []
    ...
    for line in lines:
        ...
        if not s or s.startswith("#"):
            continue
        if s.startswith(".ilb "):
            input_signals = s[5:].split()
        elif s.startswith(".ob "):
            output_signals = s[4:].split()
        elif s.startswith("."):
            continue
        else:
            parts = s.split()
            ...
            if len(in_str) != len(input_signals) or len(out_str) != len(output_signals):
                continue
            rows.extend(_expand_input_dontcares(in_str, out_str, input_signals, output_signals))
    if not input_signals:
        raise ValueError("PLA file has no '.ilb' header")
    if not output_signals:
        raise ValueError("PLA file has no '.ob' header")
    return input_signals, output_signals, rows


def _is_pla_file(path: Path) -> bool:
    """Return True if the file looks like an Espresso PLA (has .ilb or .i header)."""
    try:
        with path.open(encoding="utf-8") as f:
            for _ in range(30):
                ...
                if not line:
                    break
                s = line.strip()
                if s.startswith(".ilb ") or s.startswith(".i "):
                    return True
    except OSError:
        pass
    return False
...
def main() -> int:
    ...
    try:
        if _is_pla_file(args.truth_table):
            input_signals, output_signals, rows = parse_pla(args.truth_table)
        else:
